...
export default function Configurator3DPage() {
  ...
  useEffect(() => {
    const initPage = async () => {
      ...
      if (passedConfig && passedConfig.room) {
        console.log(
          "[configurator3d] Synchronizing state from 2D configurator:",
          passedConfig,
        );
        ...
        setTimeout(() => {
          if (prodId === "1" || prodName.toLowerCase().includes("kitchen")) {
            ...
            prodId === "2" ||
            prodName.toLowerCase().includes("lemari") ||
            prodName.toLowerCase().includes("wardrobe")
          ) {
            // Wardrobe
            addItem("item-wardrobe-2door");
          } else if (
            prodId === "4" ||
            prodName.toLowerCase().includes("tv") ||
            prodName.toLowerCase().includes("rack")
          ) {
            // TV Rack
  ...
  const handleReset = () => {
    if (
      window.confirm(
        "Apakah Anda yakin ingin menghapus semua item dan foto background di scene 3D ini?",
      )
    ) {
      resetScene();
    ...
    <div className="flex flex-col h-screen w-screen bg-slate-950 text-slate-100 overflow-hidden font-sans relative">
        ...
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <div>
            <h1 className="text-sm md:text-base font-extrabold tracking-tight text-white flex items-center gap-1.5 leading-none">
              AFANDI INTERIOR{" "}
              <span className="text-[9px] md:text-[10px] font-bold bg-teal-500/20 text-teal-400 border border-teal-500/30 px-1.5 py-0.5 rounded-full">
                3D
              </span>
            </h1>
            <p className="text-[8px] md:text-[10px] text-slate-500 font-medium mt-0.5 uppercase tracking-wider">
              Konfigurator Interior 3D Real-time
            </p>
          </div>
        </div>
          ...
          {placedItems.length > 0 && (
            <div className="hidden sm:flex items-center gap-1 bg-slate-950/80 border border-slate-800/80 rounded-xl px-3.5 py-1.5 text-xs">
              ...
              <span className="text-amber-400 font-bold">
                Rp {bom.total.toLocaleString("id-ID")}
              </span>
            </div>
          )}
          <button
          ...
          {!isMobile && !isSidebarOpen && (
            <>
              ...
              <button
                ...
                className="p-2 text-slate-400 hover:text-teal-400 hover:bg-teal-500/10 rounded-xl border border-transparent hover:border-teal-500/20 transition-all flex items-center active:scale-95 cursor-pointer"
                title="Buka Panel"
              >
                <Menu size={16} />
              </button>
      ...
      {isMobile ? (
        ...
        <div className="flex-1 relative overflow-hidden flex flex-col">
          ...
          <div className="absolute top-4 left-4 z-10 pointer-events-none bg-slate-950/75 backdrop-blur border border-slate-800/80 p-2.5 rounded-xl shadow-lg max-w-[200px]">
            <div className="text-[9px] text-slate-400 leading-relaxed font-semibold">
              <span className="text-teal-400">💡 Navigasi Sentuh:</span>
              <br />
              • Putar: 1 Jari Seret
              <br />
              • Geser: 2 Jari Seret
          ...
          >
            ...
            {isPanelExpanded && (
              <div className="flex-1 overflow-hidden">
                {activeTab === "catalog" && (
                  <CatalogPanel onAddItem={handleItemAdded} />
                )}
                {activeTab === "material" && <MaterialPanel />}
                {activeTab === "properties" && <PropertiesPanel />}
                {activeTab === "perspective" && <PerspectivePanel />}
                {activeTab === "bom" && (
                  <BOMPanel onCheckout={() => setIsCheckoutOpen(true)} />
                )}
              </div>
            )}
        ...
        <div className="flex flex-1 overflow-hidden relative">
          ...
          <div className="flex-1 h-full relative z-10">
            ...
            <div className="absolute top-4 left-4 z-20 pointer-events-none bg-slate-950/75 backdrop-blur border border-slate-800/80 p-3 rounded-xl max-w-xs shadow-xl flex items-start gap-2.5">
              ...
              <div className="text-[10px] text-slate-400 leading-relaxed">
                <strong>Kontrol Navigasi:</strong>
                <br />
                • Orbit: Klik kiri & seret mouse
                <br />
                • Pan: Klik kanan & seret / Shift + Drag
                <br />• Zoom: Scroll wheel mouse
              </div>
            </div>
          </div>
          ...
          <div
            className={`h-full flex flex-col z-20 shadow-2xl relative bg-slate-900 border-l border-slate-800 transition-all duration-300 ${isSidebarOpen ? "w-[360px] md:w-[390px]" : "w-0"} overflow-hidden`}
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800">
              <button
                onClick={() => setIsSidebarOpen(false)}
                className="w-9 h-9 flex items-center justify-center rounded-full bg-slate-950/80 border border-slate-700 text-slate-300 hover:text-white hover:border-slate-600 transition-colors mr-3"
